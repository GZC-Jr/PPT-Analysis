...
class MainWindow(QMainWindow):
    ...
    # --- 新增槽函数 ---
    def _handle_analysis_progress(self, value):
        # progress_updated信号现在会发射两个值：先是最大值，然后是当前值
        # 这是一个小技巧，用于更新进度条的范围和值
        if value > self.analysis_controls.progress_slider.maximum():
            self.analysis_controls.set_progress_range(value)
        else:
            self.analysis_controls.update_progress(value)
