...
class AnalysisControls(QWidget):
    ...
    def _init_ui(self):
        ...
        self.interpage_label = ContextMenuLabel("开启")
        interpage_layout.addLayout(self._create_switch_layout(self.interpage_label, self.interpage_switch))

        self.divergence_switch = Switch()
        self.association_switch = Switch()
        self.page_mode_group = QButtonGroup(self)
        self.page_mode_group.setExclusive(True)  # 确保互斥
    ...
    def _create_switch_layout(self, label, switch):  # label 现在必须是 QWidget
        layout = QHBoxLayout()
